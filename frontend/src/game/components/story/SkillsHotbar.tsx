/**
 * SkillsHotbar — 9 active combat skills purchased and activated during battle.
 *
 * Skill states: locked → ready → active (cooldown) → ready
 * GCD (global cooldown): 1s after any skill activation.
 * Meta-skills:
 *   - Energize: next activated skill has 2× effect.
 *   - Reload: reduces last-used skill cooldown by 50%.
 *
 * Gold cost: base_cost_gold × 1.07^level
 * Cooldown reduction per level: -5% per level (capped at -70%)
 */
...
const SkillsHotbar: React.FC<Props> = ({ session, gameConfigs }) => {
  ...
  const handleUnlock = useCallback(async (skill: SkillDef) => {
    ...
    try {
      const res = await api.post(`/api/game/story/session/${session.sessionId}/upgrade`, {
        ...
        target_id: skill.id, // Fixed: use target_id consistent with UpgradeMenu
        quantity: 1,
      });
      if (!res.ok) return;

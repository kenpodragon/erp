...
### 1.3 Current State (What Already Exists)
| Component | Status | Notes |
|-----------|--------|-------|
| Firebase Google SSO (Frontend) | ✅ Working | `signInWithPopup()` in `frontend/src/App.tsx`. Client-side only. |
| Firebase Google SSO (Admin) | ✅ Working | Same as frontend + client-side email/IP whitelist in `admin/src/App.tsx`. |
| Backend Auth Middleware | ❌ Not started | `firebase-admin` is in `requirements.txt` but not imported or used. |
| Backend Endpoints | ❌ Skeleton only | `/`, `/health`, `/hello`. No user/profile/game endpoints. |
| SQLModel Models | ❌ None defined | `SQLModel.metadata.create_all(engine)` is commented out. |
| DB Tables (Player) | ❌ None | Only book processing tables exist (`book_tables.sql`). |
| Admin Dashboard UI | ❌ Login only | No management screens beyond the SSO + whitelist gate. |

### 1.4 Dependencies
| Package | Purpose | Status |
|---------|---------|--------|
| `firebase-admin` | Backend JWT validation | In `requirements.txt`, not yet used |
| `sqlmodel` / `sqlalchemy` | ORM for all new tables | In `requirements.txt`, engine configured |
| `psycopg2-binary` | PostgreSQL driver | In `requirements.txt`, working |
| `python-dotenv` | Environment variable loading | In `requirements.txt`, working |
| `stripe` | Payment webhooks (future — not this phase) | **Not yet added** |
| `Pillow` or equivalent | Avatar image processing/resizing | **Needs to be added** |
...
- [ ] **FR-2.1:** The frontend authenticates players via **Firebase Google SSO** using `signInWithPopup()` (already implemented).
- [ ] **FR-2.2:** After successful Firebase authentication, the frontend must obtain a **Firebase ID Token** (`user.getIdToken()`) and include it as a `Bearer` token in the `Authorization` header of **every** backend API request.
- [ ] **FR-2.3:** The frontend must handle token refresh transparently — Firebase tokens expire after 1 hour. Use `onIdTokenChanged()` to auto-refresh and update the stored token.
- [ ] **FR-2.4:** If a backend request returns `401 Unauthorized`, the frontend must attempt a silent token refresh. If the refresh fails, redirect the player to the Splash page with a "Session expired" message.
- [ ] **FR-2.5:** On logout, the frontend must call `firebase.auth().signOut()`, clear all local state (tokens, player data), and redirect to the Splash page.

### 2.2 Admin Authentication

- [ ] **FR-2.6:** Admin authentication uses the same Firebase Google SSO flow as the player frontend.
...
- [ ] **FR-2.11:** Create a FastAPI dependency `get_current_player()` that:
...
- [ ] **FR-2.12:** Create a FastAPI dependency `get_current_admin()` that:
...
- [ ] **FR-2.13:** All `/api/players/` and `/api/game/` routes must use `get_current_player()` as a dependency.
- [ ] **FR-2.14:** All `/api/admin/` routes must use `get_current_admin()` as a dependency.
- [ ] **FR-2.15:** Firebase Admin SDK must be initialized on backend startup using the service account credentials (via `GOOGLE_APPLICATION_CREDENTIALS` env var or `FIREBASE_CREDENTIALS` JSON env var).
...
- [ ] **FR-3.1:** When a player authenticates for the **first time**, the backend must create a `players` record populated from the Firebase token data (`firebase_uid`, `email`, Google `display_name`, Google `avatar_url`).
- [ ] **FR-3.2:** When a player authenticates on **subsequent visits**, the backend must update `last_login_at` and optionally sync the Google `email` / `avatar_url` if they've changed.
- [ ] **FR-3.3:** Players must be able to set a **custom display name (alias)** that is different from their Google name. This alias is what other players see in leaderboards, chat, etc.
...
- [ ] **FR-4.1:** The game supports **4-6 character classes** derived from the *Towers of Elysium* lore. Exact class names, descriptions, and stat distributions must be determined during implementation by consulting `../Books/BOOKS.md` and the extracted entity/narrative data in the DB.
- [ ] **FR-4.2:** Each class is defined by:
...
- [ ] **FR-4.3:** Character classes are **seeded via SQL** in the migration script. They are **not** player-created. Admins can edit class definitions (stats, lore, availability) via the Admin panel.

### 4.2 Character Creation

- [ ] **FR-4.4:** Each player may create **one character** for the MVP. (Multi-character support is deferred — the DB schema supports it, but the UI enforces a limit of 1.)
- [ ] **FR-4.5:** Character creation requires:
...
- [ ] **FR-4.6:** On character creation, the backend must:
...
- [ ] **FR-4.7:** Character names **cannot be changed** after creation (for MVP). Display name (alias) on the player profile is the mutable identity.

### 4.3 Character API Endpoints

- [ ] **FR-4.8:** `POST /api/players/me/characters`
  - **Auth:** `get_current_player()`
...
- [ ] **FR-5.1:** The Splash page is accessible at `/` (root URL) for unauthenticated users.
- [ ] **FR-5.2:** Visual design: dark fantasy aesthetic consistent with the *Towers of Elysium* theme.
...
- [ ] **FR-5.3:** Primary CTA button: **"Begin Your Ascent"** (or similar lore-flavored text). Triggers Firebase Google SSO popup.
- [ ] **FR-5.4:** Secondary content (below fold or accessible via scroll/links):
...
- [ ] **FR-5.5:** Footer: links to Terms of Service, Privacy Policy, and a "Contact Support" link.
- [ ] **FR-5.6:** If a player is **already authenticated** (valid Firebase session in local storage), skip the Splash and route directly to Home Base (or Character Creation if they haven't completed onboarding).
- [ ] **FR-5.7:** Mobile-responsive layout. All content readable and CTA accessible on screens down to 360px width.
...
- [ ] **FR-5.8:** On successful Firebase auth, the frontend calls `POST /api/auth/login`.
...
- [ ] **FR-5.9:** If `terms_accepted_at` is `NULL` → show Terms acceptance before any other step.

#### Step 1: Terms of Service Acceptance
- [ ] **FR-5.10:** Display a modal or full-page overlay with the Terms of Service and Privacy Policy text.
- [ ] **FR-5.11:** Player must scroll to the bottom (or check a checkbox) AND click "I Accept" to proceed.
- [ ] **FR-5.12:** On acceptance, call `POST /api/players/me/accept-terms`. Block progression until the API confirms.
- [ ] **FR-5.13:** The Terms text itself should be stored as a static asset or fetched from a CMS/config endpoint (so it can be updated without redeploying). For MVP, a static markdown file rendered in-app is acceptable.

#### Step 2: Profile Setup
- [ ] **FR-5.14:** Display the player's Google name and avatar as defaults.
- [ ] **FR-5.15:** Allow the player to set a custom **alias** (display name). Show validation feedback in real-time (length, allowed characters, uniqueness check via debounced API call).
- [ ] **FR-5.16:** Allow the player to upload a custom avatar or select from preset avatars. Show a preview.
- [ ] **FR-5.17:** "Continue" button calls `PATCH /api/players/me` with the alias and avatar choice.
- [ ] **FR-5.18:** This step is **skippable** — if the player clicks "Skip," they keep their Google name/avatar and can customize later from Settings.

#### Step 3: Character Creation
- [ ] **FR-5.19:** Fetch class list from `GET /api/game/classes`.
- [ ] **FR-5.20:** Display class selection as cards: each card shows class name, lore blurb (2-3 sentences), stat distribution (STR/AGI/INT) as a visual bar or radar chart, and the class sprite (placeholder for MVP).
- [ ] **FR-5.21:** Player enters a character name. Real-time validation (length, format, uniqueness via debounced API call or on-submit).
- [ ] **FR-5.22:** "Create Character" button calls `POST /api/players/me/characters`. On success, transition to Step 4.
- [ ] **FR-5.23:** On validation error (name taken, profanity detected, etc.), display the error inline without losing the player's input.

#### Step 4: Welcome / Tutorial Prompt
- [ ] **FR-5.24:** Brief "Welcome to Elysium" interstitial screen. Displays the character name, class, and a lore-flavored welcome message.
- [ ] **FR-5.25:** Two CTAs: **"Begin Adventure"** (routes to the game / Chapter 1) and **"Explore Home Base"** (routes to Home Base).
- [ ] **FR-5.26:** This screen is shown **once** after character creation. Returning players never see it.
...
- [ ] **FR-5.27:** Home Base is the default post-login route for returning players (`/home` or `/`).
- [ ] **FR-5.28:** Display:
...
- [ ] **FR-5.29:** Navigation to (as placeholders for future implementation):
...
- [ ] **FR-5.31:** Mobile-responsive. Home Base must work on mobile browsers.
...
- [ ] **FR-6.1:** Players access support from the Home Base via a "Support" link and from the footer of every page.
- [ ] **FR-6.2:** **Submit Ticket** form:
  ...
  - **Attachments** (optional): Up to 3 files, max 5MB each. Accepted types: JPEG, PNG, GIF, PDF, TXT.
  - **Priority** (auto-assigned): All player-submitted tickets default to `normal`. Admins can escalate.
- [ ] **FR-6.3:** **My Tickets** view:
...
- [ ] **FR-6.4:** **Ticket Detail** view (player):
  ...
  - Reply form: text area (20-5000 chars) + optional attachments (same limits as submission).
  - Players can reply to `open` and `in_progress` tickets. Cannot reply to `closed` tickets unless they reopen.
- [ ] **FR-6.5:** **Reopen Ticket:** Players can reopen a `resolved` or `closed` ticket by clicking "Reopen" and providing a reason (required text, 10-500 chars). Status returns to `in_progress`.
- [ ] **FR-6.6:** **Auto-close:** Tickets in `resolved` status for 7 days without player response are automatically closed. (Backend cron job or on-access check.)

### 6.4 Admin-Facing Requirements

- [ ] **FR-6.7:** **Ticket Queue** (admin dashboard):
...
- [ ] **FR-6.10:** **Ticket Stats** (on admin dashboard home):
  - Open tickets count, In-progress count, average resolution time, tickets created today/this week.

### 6.5 Support API Endpoints

- [ ] **FR-6.11:** `POST /api/support/tickets` — Player creates a ticket.
- [ ] **FR-6.12:** `GET /api/support/tickets` — Player lists their own tickets (paginated).
- [ ] **FR-6.13:** `GET /api/support/tickets/{ticket_id}` — Player views ticket detail (must be owner).
- [ ] **FR-6.14:** `POST /api/support/tickets/{ticket_id}/replies` — Player or admin posts a reply.
- [ ] **FR-6.15:** `PATCH /api/support/tickets/{ticket_id}/reopen` — Player reopens a resolved/closed ticket.
- [ ] **FR-6.16:** `GET /api/admin/support/tickets` — Admin lists all tickets (paginated, filterable).
- [ ] **FR-6.17:** `PATCH /api/admin/support/tickets/{ticket_id}` — Admin updates priority, status, assignment.
- [ ] **FR-6.18:** `POST /api/admin/support/tickets/{ticket_id}/notes` — Admin adds internal note.
- [ ] **FR-6.19:** `POST /api/support/tickets/{ticket_id}/attachments` — Upload attachment (player or admin).
- [ ] **FR-6.20:** `GET /api/support/attachments/{attachment_id}` — Download attachment (authorized access only).
...
#### Game Tuning
- [ ] **FR-8.1:** The following game settings must be configurable:
...
- [ ] **FR-8.2:** The following operational settings must be configurable:
...
- [ ] **FR-8.3:** Config values are stored in the `server_config` table (see [Schema doc §4](1_ONBOARDING_INIT_SCHEMA.md#4-server-config-table)). Each entry has a key, value, type, category, description, and default. Values are stored as strings and cast by the application based on `value_type`.
- [ ] **FR-8.4:** Config values are **seeded** on first deployment via SQL migration with all default values.
- [ ] **FR-8.5:** The backend must **cache** config values in memory (dictionary) on startup and refresh on a configurable interval (default: 60 seconds) or on-demand when an admin changes a value.

### 8.4 Config API & Admin UI

- [ ] **FR-8.6:** `GET /api/admin/config` — Return all config keys grouped by category. Include key, value, value_type, description, default_value, updated_at, updated_by.
- [ ] **FR-8.7:** `PATCH /api/admin/config/{key}` — Update a single config value. Validate against `value_type`. Log the change in the admin audit log (old value → new value).
- [ ] **FR-8.8:** `POST /api/admin/config/{key}/reset` — Reset a config value to its default. Log the change.
- [ ] **FR-8.9:** Admin UI: Two-tab layout (Game Settings, Operational Settings). Each setting displayed as a labeled form control (toggle for booleans, number input for integers/numerics, text input for strings, textarea for text). Save button per-setting or a global "Save All Changes" with diff preview.
- [ ] **FR-8.10:** Admin UI: "Reset to Default" button per setting. Confirmation required.
- [ ] **FR-8.11:** Maintenance mode must take effect **immediately** — the backend config cache must be invalidated when `ops.maintenance_mode` is changed.
- [ ] **FR-8.12:** `GET /api/config/public` — **Unauthenticated** endpoint returning only settings the frontend needs: `ops.maintenance_mode`, `ops.maintenance_message`, `ops.announcement_banner`, `ops.announcement_banner_type`, `ops.registration_open`. No game tuning values exposed.
...
| `support_attachments` | §3 (Schema) | File attachments on tickets/replies. |
...
### 10.2 SQL Migration
- [ ] **FR-10.1:** All tables must be created via a single SQL migration file: `/db/007_onboarding_and_admin.sql`.
- [ ] **FR-10.2:** The migration must include seed data for `character_classes` and `server_config` (see Schema doc §9).
- [ ] **FR-10.3:** The migration must include all indexes (see Schema doc §6).
- [ ] **FR-10.4:** The migration must be idempotent (`CREATE TABLE IF NOT EXISTS`, `CREATE INDEX IF NOT EXISTS`).
...
### Route Guards
- [ ] **FR-11.1:** An auth route guard checks Firebase session. If not authenticated → redirect to `/`.
- [ ] **FR-11.2:** A terms guard checks `terms_accepted_at`. If null → redirect to `/onboarding/terms`.
- [ ] **FR-11.3:** A character guard checks if player has a character. If not → redirect to `/onboarding/character`.
- [ ] **FR-11.4:** Guards execute in order: Auth → Terms → Character. The first failing guard takes precedence.
...
- [ ] **NFR-7:** The `activity_events` table will grow large. Include a `created_at` index and plan for a future retention policy (e.g., archive events older than 90 days). Not required for MVP but the schema should support it.
- [ ] **NFR-8:** All SQL for this phase goes in `/db/007_onboarding_and_admin.sql` per project convention.
- [ ] **NFR-9:** All sensitive operations (ban, unban, config changes) must be recorded in the admin audit log before the response is returned (synchronous write for audit trail integrity).
- [ ] **NFR-10:** Support ticket attachments must be stored outside the database (filesystem or GCS). Only the path is stored in the DB.

---

## 14. Error Handling

- [ ] **FR-14.1:** Invalid or expired Firebase tokens → `401 Unauthorized` with `{ error: "Invalid or expired authentication token" }`.
- [ ] **FR-14.2:** Banned player attempting API access → `403 Forbidden` with `{ error: "Account suspended. Contact support." }`.
- [ ] **FR-14.3:** Admin access denied → `403 Forbidden` with `{ error: "Access denied" }` (no detail about which check failed).
- [ ] **FR-14.4:** Alias or character name already taken → `409 Conflict` with `{ error: "This name is already taken", field: "alias|character_name" }`.
- [ ] **FR-14.5:** Player already has max characters → `409 Conflict` with `{ error: "Character limit reached" }`.
- [ ] **FR-14.6:** Validation errors (bad format, too long, etc.) → `422 Unprocessable Entity` with `{ error: "Validation failed", details: [...] }`.
- [ ] **FR-14.7:** Maintenance mode active → `503 Service Unavailable` with `{ error: "<maintenance message from config>" }`.
- [ ] **FR-14.8:** All error responses must follow a consistent JSON shape: `{ error: string, details?: any, field?: string }`.

---

## 15. Implementation Order (Suggested)

The recommended build order within this requirements document:

1. **Database migration** ([Schema doc](1_ONBOARDING_INIT_SCHEMA.md)) — Create all tables, seed character classes and config values.
2. **Backend auth middleware** (§2.3) — `get_current_player()` and `get_current_admin()`. Everything depends on this.
3. **Player profile API** (§3.2) — Login, profile CRUD, avatar upload, settings.
4. **Character API** (§4.3) — Class listing, character creation.
5. **Frontend: Splash page** (§5.1) — New public landing page.
6. **Frontend: Onboarding flow** (§5.2) — Terms → Profile → Character → Welcome.
7. **Frontend: Home Base** (§5.3) — Authenticated hub with stubs.
8. **Server config system** (§8) — DB-driven config + admin UI.
9. **Support ticket system** (§6) — Player + admin ticket UIs and API.
10. **Admin: User management** (§7) — Player list, detail, ban/unban.
11. **Activity events & audit log** (§9) — Instrumentation + analytics dashboard.
12. **Admin: Move IP/email check to backend** (§2.2) — Harden admin auth.

...
- [x] Confirm migration 020 has been applied successfully.
- [x] Back up production DB before applying.
- [x] Run `021_idle_training_schema.sql` first; verify table/column creation.
- [x] Run `022_idle_training_seed_actions.sql`; verify row counts (8 Attack, 9 Magic, 9 Lore, 8 Precision = 34 total skill_actions rows).
- [x] Identify correct scene IDs for Magic/Lore/Precision unlock gates (see §3.6).
- [x] Apply deferred scene ID updates.
- [x] Update `db/data_dictionary.md` with all changes from §4.
